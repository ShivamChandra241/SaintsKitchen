...
double cartSubtotal(List<CartItem> cart) =>
    cart.fold(0, (s, i) => s + i.totalPrice);
        ...
        child: Stack(
          children: [
            ...
            const Positioned(
              ...
              child: Center(
                child: Text(
                  "v7.0.0 • Made with ❤️ by Fantastic 6",
...
class _LoginScreenState extends State<LoginScreen> {
  ...
  Future<void> _login() async {
    ...
    await prefs.setString('id', _idCtrl.text.trim());
  ...
  void addToCart(FoodItem item, FoodVariant? variant, int qty) {
    ...
    setState(() {
      if (existing != -1) {
        ...
        cart.add(CartItem(item, qty, selectedVariant: variant));
      }
    });
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  style: ElevatedButton.styleFrom(
                      ...
                      foregroundColor: Colors.white),
                  onPressed: () {
                    widget.onAddToCart(item, selectedVar, qty);
...
class OrdersPage extends StatefulWidget {
  ...
  const OrdersPage(
      {super.key, required this.currentCart, required this.onClearCart});
...
class _OrdersPageState extends State<OrdersPage> {
  ...
  Future<void> _loadHistory() async {
    ...
    if (hList != null) {
      setState(() {
        orderHistory = (jsonDecode(hList) as List)
  ...
  void _applyCoupon() {
    if (_couponCtrl.text.toUpperCase() == "SAINTS50") {
      ...
      ScaffoldMessenger.of(context)
          .showSnackBar(const SnackBar(content: Text("Coupon Applied! ₹50 OFF")));
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Future<void> _placeOrder() async {
    ...
    final double subtotal = cartSubtotal(widget.currentCart);
          ...
          child: Column(mainAxisSize: MainAxisSize.min, children: [
            TweenAnimationBuilder(
              ...
              builder: (context, double value, child) =>
                  Transform.scale(scale: value, child: const Icon(Icons.check_circle, color: Colors.green, size: 80)),
            ),
            const SizedBox(height: 20),
    ...
    if (tList != null) {
      transactions = (jsonDecode(tList) as List)
    ...
    List<Order> history = [];
    if (hList != null) {
      history = (jsonDecode(hList) as List)
          .map((i) => Order.fromJson(i))
          .toList();
    }
    history.add(newOrder);
    await prefs.setString(
        'order_history', jsonEncode(history.map((e) => e.toJson()).toList()));
    ...
    ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("₹${finalTotal.toInt()} deducted from wallet")));
  }

  void _rateOrder(Order o, int stars) async {
    setState(() => o.rating = stars);
    final prefs = await SharedPreferences.getInstance();
    final hList = prefs.getString('order_history');
    if (hList != null) {
      List<Order> history = (jsonDecode(hList) as List)
          .map((i) => Order.fromJson(i))
          .toList();
      final idx = history.indexWhere((e) => e.id == o.id);
      if (idx != -1) {
        history[idx].rating = stars;
        await prefs.setString(
            'order_history', jsonEncode(history.map((e) => e.toJson()).toList()));
      }
    }
    ...
    return DefaultTabController(
      ...
      child: Scaffold(
        appBar: AppBar(
          ...
          bottom: const TabBar(tabs: [Tab(text: "Cart"), Tab(text: "Track Orders")]),
        ),
        body: TabBarView(
          children: [
            // CART TAB
                          ...
                          return ListTile(
                            ...
                            subtitle: Row(children: [
                              IconButton(
                                  icon: const Icon(Icons.remove),
                                  onPressed: () =>
                                      setState(() => ci.quantity = max(1, ci.quantity - 1))),
                              Text("${ci.quantity}"),
                              IconButton(
                                  icon: const Icon(Icons.add),
                                  onPressed: () => setState(() => ci.quantity++)),
                            ]),
                      ...
                      child: Column(children: [
                        ...
                        Row(
                            ...
                            children: [
                              const Text("Total"),
                              Text(
                                "₹${max(0, cartSubtotal(widget.currentCart) - discount).toInt()}",
                                style: const TextStyle(
                    ...
                    children: [
                      Padding(
                        ...
                        child: Column(children: [
                          ...
                          const SizedBox(height: 10),
                          if (ready) ...[
                            QrImageView(data: o.id, size: 100),
                            const SizedBox(height: 6),
...
class _WalletPageState extends State<WalletPage> {
  ...
  void _showTxnDetails(WalletTransaction t) {
    showDialog(
      context: context,
...
class _DepositSheetState extends State<DepositSheet> {
  ...
  Future<void> _pay() async {
    ...
    final tList = prefs.getString('transactions');
    List<WalletTransaction> ts = [];
    if (tList != null) {
      ts = (jsonDecode(tList) as List)
          .map((i) => WalletTransaction.fromJson(i))
          .toList();
    }
    ...
    await prefs.setString(
        'transactions', jsonEncode(ts.map((e) => e.toJson()).toList()));
